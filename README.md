...
export PRINTER_CUPS_TEXT_ALIGN=center # center / left
export PRINTER_CUPS_WIDTH_PX=576
...
`image` モードではフォントサイズは固定で、コンテンツ量に応じて印刷画像の高さを自動で伸ばします。  
...
export AUDIO_TEMPO_WINDOW_SEC=5.0
export WEB_DASHBOARD_ENABLED=1
export WEB_DASHBOARD_HOST=0.0.0.0
export WEB_DASHBOARD_PORT=5000
export WEB_SAMPLE_INTERVAL_SEC=0.5
export PIPELINE_PROCESS_INTERVAL_SEC=0.1
python3 -m jetson.main
```

別端末からは `http://<jetson-ip>:5000` にアクセスします。

`tempo_bpm` が動きにくい場合は、`PIPELINE_PROCESS_INTERVAL_SEC` を `0.05` まで下げ、`AUDIO_TEMPO_WINDOW_SEC` を `3.0`〜`5.0` で調整してください。

セッション集約時の `tempo_bpm` は平均ではなく最大値を採用しています。

万博モチーフ（国・パビリオン・旅先）は [`docs/expo_catalog.json`](docs/expo_catalog.json) から読み込みます。
パスを変える場合:

```bash
export EXPO_CATALOG_PATH=/path/to/expo_catalog.json
```

Googleスプレッドシートから再生成する場合:

```bash
python3 tools/build_expo_catalog.py
```

この再生成では、`cat=外` から万博モチーフ、`cat=食` からおすすめレストランを作成し、`note.com` 記事からおすすめ料理リストを抽出します。
...
AI_PROMPT_PROFILE=omikuji_v1
AI_NARRATIVE_CONTEXT=2025大阪・関西万博をモチーフにした音と身体のおみくじ
AI_TONE=やわらかく詩的、ただし具体
AI_MIN_CHARS=120
AI_MAX_CHARS=220
```

クラウド文面の世界観調整は `AI_NARRATIVE_CONTEXT` と `AI_TONE` を変更するのが最短です。
