# 音と身体から未来を生成するおみくじシステム 仕様書 v0.3

## 1. 目的

人の声と身体の動きを入力として取得し、ローカルで特徴量を抽出したうえでクラウドAIが意味づけを行い、短い未来トピックとしてのおみくじ文を生成する。Jetson Nano は印刷データを生成して Raspberry Pi 上の印刷システムへ送信し、最終的な出力はサーマルプリンタで紙として返す。
...
- クラウドAIへの送信
- Web可視化
- Raspberry Pi 印刷サーバへのジョブ送信

### 3.3 Raspberry Pi + サーマルプリンタ
...
[M5Stack] -> [Jetson Nano] -> [Cloud AI]
                   |              |
                   +-> [Web UI]   |
                   +-> [Raspberry Pi Print Server] -> [Thermal Printer]
```
...
Jetson は整形済みの印刷テキストとメタデータを Raspberry Pi に送る。Raspberry Pi 側で実際のサーマルプリンタ制御を行う。
...
Jetson から Raspberry Pi へ送る印刷ジョブの例:
...
- Jetson と Raspberry Pi は Ethernet または Wi-Fi で同一ネットワークに接続する
- Jetson から Raspberry Pi の HTTP エンドポイントへ `POST` する
- エンドポイント例: `http://raspberrypi.local:8000/print-jobs`
...
3. `jetson/printer/printer.py` を Raspberry Pi 印刷サーバ接続へ合わせる
